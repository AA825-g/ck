...
case 'editPrice':
	...
	if($DB->query("UPDATE `shua_tools` SET `price`='$price',`cost`='$cost',`cost2`='$cost2',`prid`='$prid' WHERE `tid`='{$tid}'"))
...
case 'order':
	...
	if(strpos($rows['tradeno'],'kid')!==false){
		...
		if($trade['type']=='rmb')$addstr.='<li class="list-group-item"><b>支付用户ID：</b>'.(is_numeric($rows['userid'])&&$rows['userid']!=$rows['zid']?'<a href ="userlist.php?zid='.$rows['userid'].'" target="_blank">'.$rows['userid'].'</a>':'<a href ="sitelist.php?zid='.$rows['zid'].'" target="_blank">'.$rows['zid'].'</a>').'</li>';
	}else{
		$trade['type']='默认';
	}
...
case 'kms':
	$id=intval($_GET['id']);
	$rows=$DB->get_row("select * from shua_faka where kid='$id' limit 1");
	if(!$rows)
...
case 'refund': //退款操作
	...
	if(is_numeric($row['userid']))$zid = intval($row['userid']);
	else $zid = $row['zid'];
	$DB->query("update `shua_site` set `rmb`=`rmb`+{$money} where `zid`='{$zid}'");
	rollbackPoint($id);
	addPointRecord($row['zid'], $money, '退款', '订单(ID'.$id.')已退款到分站余额');
	$DB->query("update shua_orders set status='4',result=NULL where id='{$id}'");
...
case 'delworkorder':
	$id=intval($_GET['id']);
	$sql="DELETE FROM shua_workorder WHERE id='$id' limit 1";
	if($DB->query($sql)){
		exit('{"code":0,"msg":"删除成功！"}');
	}else{
		exit('{"code":-1,"msg":"删除失败！'.$DB->error().'"}');
