	...
	if($conf['tongji_time']>0){
		...
		if($tongji_cachetime+intval($conf['tongji_time'])>=time() && $tongji_cache){
			if($conf['shoppingcart']==1){
				$cart_count = $DB->count("SELECT count(*) from shua_cart WHERE userid='$cookiesid' AND status<=1");
			}
...
case 'pay':
	...
	if($method == 'cart_edit'){
		...
		$cart_item = $DB->get_row("SELECT * FROM `shua_cart` WHERE `id`='$shop_id' LIMIT 1");
		if(!$cart_item)exit('{"code":-1,"msg":"商品不存在！"}');
		if($cart_item['userid']!=$cookiesid || $cart_item['status']>1)exit('{"code":-1,"msg":"商品权限校验失败"}');
		$tool=$DB->get_row("select * from shua_tools where tid='{$cart_item['tid']}' limit 1");
	}else{
		$tid=intval($_POST['tid']);
	...
	if($tool && $tool['active']==1){
		...
		if($method == 'cart_add'){
			$sql="insert into `shua_cart` (`userid`,`zid`,`tid`,`input`,`num`,`money`,`addtime`,`status`) values ('".$cookiesid."','".($siterow['zid']?$siterow['zid']:1)."','".$tid."','".$input."','".$num."','".$need."','".$date."','0')";
			if($DB->query($sql)){
				$cart_count = $DB->count("SELECT count(*) from shua_cart WHERE userid='$cookiesid' AND status<=1");
			...
			$sql="update `shua_cart` set `input`='$input',`num`='$num',`money`='$need',`status`='0' where id='$shop_id'";
			if($DB->query($sql)){
...
case 'cart_buy':
	...
	foreach($shop_ids as $shop_id){
		$cart_item = $DB->get_row("SELECT * FROM `shua_cart` WHERE `id`='".intval($shop_id)."' LIMIT 1");
		if(!$cart_item)exit('{"code":-1,"msg":"商品不存在！"}');
		if($cart_item['userid']!=$cookiesid || $cart_item['status']>1)exit('{"code":-1,"msg":"商品权限校验失败"}');
		if($cart_item['money']=='0' || !preg_match('/^[0-9.]+$/', $cart_item['money']))exit('{"code":-1,"msg":"商品金额不合法"}');
		$ids[] = intval($shop_id);
		$allmoney += floatval($cart_item['money']);
		$DB->query("update `shua_cart` set `status`=1 where `id`='{$cart_item['id']}'");
	}
	if(count($ids)==0)exit('{"code":-1,"msg":"您未在购物车添加任何商品"}');
	$toolname=$DB->get_column("select name from shua_tools where tid='{$cart_item['tid']}' limit 1");
	$toolname = $toolname.'等多件';
	$input = implode('|',$ids);
	$trade_no=date("YmdHis").rand(111,999);

	$sql="insert into `shua_pay` (`trade_no`,`tid`,`zid`,`input`,`num`,`name`,`money`,`ip`,`userid`,`inviteid`,`addtime`,`status`) values ('".$trade_no."','-3','".($siterow['zid']?$siterow['zid']:1)."','".$input."','".count($ids)."','".$toolname."','".$allmoney."','".$clientip."','".$cookiesid."','".$invite_id."','".$date."','0')";
	if($DB->query($sql)){
		unset($_SESSION['addsalt']);
		if($conf['forcermb']==1){$conf['alipay_api']=0;$conf['wxpay_api']=0;$conf['qqpay_api']=0;}
...
case 'cart_cancel':
	...
	if($srow['status']==0){
		...
		$ids = implode(',',$input);
		$DB->query("UPDATE shua_cart SET status=0 WHERE id IN ($ids) AND status=1");
		if($conf['verify_open']==1){
			$_SESSION['addsalt'] = $hashsalt;
		}
	}
...
break;
case 'cart_empty':
	if($DB->query("DELETE FROM shua_cart WHERE userid='$cookiesid' AND status=0")){
...
case 'cart_shop_del':
	$id = intval($_POST['id']);
	$cart_item = $DB->get_row("SELECT * FROM `shua_cart` WHERE `id`='$id' LIMIT 1");
	if(!$cart_item)exit('{"code":-1,"msg":"商品不存在！"}');
	if($cart_item['userid']!=$cookiesid || $cart_item['status']>1)exit('{"code":-1,"msg":"商品权限校验失败"}');
	if($DB->query("DELETE FROM shua_cart WHERE id='$id'")){
