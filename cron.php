...
if($_GET['do']=='pricejk'){
	...
	$success = 0;
	$is_need = 0;
	$rs=$DB->query("SELECT * FROM shua_shequ WHERE type IN (0,1,2,11,12) ORDER BY id ASC");
...
elseif($_GET['do']=='daily'){
	...
	$sq3 = $DB->affected();
	$DB->query("DELETE FROM `shua_cart` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-12 hours"))."' and status<2");
	$sq4 = $DB->affected();
